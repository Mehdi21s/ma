...
def init_db():
    ...
    # Backward-compatible migrations for user/contact tracking.
...
def mark_start(user_id):
    conn = get_db()
    conn.execute(
        "UPDATE users SET started_at = COALESCE(started_at, CURRENT_TIMESTAMP), start_count = COALESCE(start_count, 0) + 1 WHERE user_id = ?",
        (user_id,),
    )
    conn.commit()
    conn.close()
...
def save_user_phone(user_id, phone):
    conn = get_db()
    conn.execute(
        "UPDATE users SET phone = ?, phone_shared_at = CURRENT_TIMESTAMP WHERE user_id = ?",
        (phone, user_id),
    )
    conn.commit()
    conn.close()
...
def admin_user_stats():
    conn = get_db()
    total = conn.execute("SELECT COUNT(*) FROM users WHERE started_at IS NOT NULL").fetchone()[0]
    with_phone = conn.execute("SELECT COUNT(*) FROM users WHERE phone IS NOT NULL AND phone != ''").fetchone()[0]
    today = conn.execute("SELECT COUNT(*) FROM users WHERE started_at IS NOT NULL AND date(started_at, 'localtime') = date('now', 'localtime')").fetchone()[0]
    total_starts = conn.execute("SELECT COALESCE(SUM(start_count), 0) FROM users").fetchone()[0]
    conn.close()
    return total, with_phone, today, total_starts


def list_phone_users(limit=15, offset=0):
    conn = get_db()
    rows = conn.execute(
        "SELECT user_id, username, first_name, phone, started_at, phone_shared_at FROM users WHERE phone IS NOT NULL AND phone != '' ORDER BY phone_shared_at DESC LIMIT ? OFFSET ?",
        (limit, offset),
    ).fetchall()
    total = conn.execute("SELECT COUNT(*) FROM users WHERE phone IS NOT NULL AND phone != ''").fetchone()[0]
    conn.close()
    return rows, total


def list_started_users(limit=15, offset=0):
    conn = get_db()
    rows = conn.execute(
        "SELECT user_id, username, first_name, phone, started_at, start_count FROM users WHERE started_at IS NOT NULL ORDER BY started_at DESC LIMIT ? OFFSET ?",
        (limit, offset),
    ).fetchall()
    total = conn.execute("SELECT COUNT(*) FROM users WHERE started_at IS NOT NULL").fetchone()[0]
    conn.close()
...
def admin_panel_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="📊 آمار", callback_data="new_admin_stats"),
            InlineKeyboardButton(text="👥 کاربران استارت‌زده", callback_data="started_users:0"),
        ],
        [
            InlineKeyboardButton(text="📱 کاربران دارای شماره", callback_data="phone_users:0"),
        ],
        [
            InlineKeyboardButton(text="🔐 مدیریت عضویت اجباری", callback_data="join_manage"),
        ],
    ])


def admin_panel_text():
    total, with_phone, today, total_starts = admin_user_stats()
    return (
        "👑 <b>پنل مدیریت</b>\n\n"
        f"👥 کاربران استارت‌زده: <b>{total}</b>\n"
        f"📱 کاربران دارای شماره: <b>{with_phone}</b>\n"
        f"📅 استارت امروز: <b>{today}</b>\n"
        f"🔁 مجموع استارت‌ها: <b>{total_starts}</b>"
    )
...
async def admin_command_panel(message: Message):
    if not is_admin(message.from_user.id):
        return
    await message.answer(
        admin_panel_text(),
...
@dp.callback_query(F.data == "new_admin_stats")
async def new_admin_stats(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
...
@dp.callback_query(F.data.startswith("started_users:"))
async def started_users_panel(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    try:
        ...
        page = 0
    limit = 10
    rows, total = list_started_users(limit, page * limit)
    if not rows:
        text = "👥 هنوز کاربری /start نزده است."
    else:
        lines = [f"👥 <b>کاربرانی که /start زده‌اند</b>\nصفحه {page + 1} از {max(1, (total + limit - 1) // limit)}\n"]
        for r in rows:
            username = f"@{escape(r['username'])}" if r['username'] else "بدون یوزرنیم"
            phone = escape(r['phone']) if r['phone'] else "ثبت نشده"
            lines.append(
                f"👤 {escape(r['first_name'] or 'بدون نام')}\n"
                f"🆔 <code>{r['user_id']}</code>\n"
                f"🔗 {username}\n"
                f"📱 {phone}\n"
                f"🔁 استارت: {r['start_count'] or 0}\n"
                f"🕐 {r['started_at'] or '-'}\n"
                "────────────"
            )
        text = "\n".join(lines)
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️ قبلی", callback_data=f"started_users:{page-1}"))
    if (page + 1) * limit < total:
        nav.append(InlineKeyboardButton(text="بعدی ➡️", callback_data=f"started_users:{page+1}"))
    buttons = [nav] if nav else []
    buttons.append([InlineKeyboardButton(text="🔙 پنل مدیریت", callback_data="new_admin_home")])
    await callback.answer()
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))


@dp.callback_query(F.data.startswith("phone_users:"))
async def phone_users_panel(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    try:
        ...
        page = 0
    limit = 10
    rows, total = list_phone_users(limit, page * limit)
    if not rows:
        text = "📱 هنوز شماره‌ای با ارسال Contact ثبت نشده است."
    else:
        lines = [f"📱 <b>کاربران دارای شماره</b>\nصفحه {page + 1} از {max(1, (total + limit - 1) // limit)}\n"]
        for r in rows:
            username = f"@{escape(r['username'])}" if r['username'] else "بدون یوزرنیم"
            lines.append(
                f"👤 {escape(r['first_name'] or 'بدون نام')}\n"
                f"🆔 <code>{r['user_id']}</code>\n"
                f"🔗 {username}\n"
                f"📱 <code>{escape(r['phone'])}</code>\n"
                f"🕐 ثبت شماره: {r['phone_shared_at'] or '-'}\n"
                "────────────"
            )
        text = "\n".join(lines)
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️ قبلی", callback_data=f"phone_users:{page-1}"))
    if (page + 1) * limit < total:
        nav.append(InlineKeyboardButton(text="بعدی ➡️", callback_data=f"phone_users:{page+1}"))
    buttons = [nav] if nav else []
    buttons.append([InlineKeyboardButton(text="🔙 پنل مدیریت", callback_data="new_admin_home")])
    await callback.answer()
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
@dp.callback_query(F.data == "new_admin_home")
async def new_admin_home(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
        return
    await callback.answer()
    await callback.message.edit_text(admin_panel_text(), parse_mode="HTML", reply_markup=admin_panel_keyboard())


@dp.message(F.contact)
async def contact_capture_handler(message: Message):
    user = message.from_user
    register_user(user.id, user.username, user.first_name)
    contact = message.contact
    if not contact.user_id or contact.user_id != user.id:
        await message.answer("❌ لطفاً فقط شماره تلفن خودتان را با دکمه ارسال شماره بفرستید.")
        return
    save_user_phone(user.id, contact.phone_number)
    # Notify configured/root admins.
    for admin_id in ROOT_ADMIN_IDS:
        try:
            await bot.send_message(
                admin_id,
                "📱 <b>شماره جدید ثبت شد</b>\n\n"
                f"👤 نام: {escape(user.first_name or '-') }\n"
                f"🔗 یوزرنیم: {escape('@' + user.username if user.username else 'بدون یوزرنیم')}\n"
                f"🆔 آیدی عددی: <code>{user.id}</code>\n"
                f"📱 شماره: <code>{escape(contact.phone_number)}</code>",
                parse_mode="HTML",
            )
        except Exception:
            pass
    await message.answer("✅ شماره شما با موفقیت ثبت شد.", reply_markup=main_keyboard(user.id))
