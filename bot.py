...
ADMIN_ID = int(os.getenv("ADMIN_ID", "8718566270"))
...
async def admin_callback(callback: CallbackQuery):
    admin_message_mode.add(callback.from_user.id)
    await callback.message.answer(
        "📩 <b>ارتباط با ادمین</b>\n\n"
...
async def text_handler(message: Message, bot: Bot):
    ...
    # Message to admin mode: forward the user's message to the current admin.
    if message.from_user.id in admin_message_mode:
        admin_message_mode.discard(message.from_user.id)
        user = message.from_user
        ...
        try:
            ...
            await message.answer("✅ پیام شما برای ادمین ارسال شد.", reply_markup=main_keyboard())
        except Exception:
            await message.answer("❌ ارسال پیام انجام نشد. دوباره تلاش کنید.", reply_markup=main_keyboard())
        return
